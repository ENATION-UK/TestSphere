...
export default {
  ...
  mounted() {
    ...
    if (this.dragHandle) {
      ;[...this.$el.querySelectorAll(this.dragHandle)].forEach((dragHandle) => {
        dragHandle.setAttribute('data-drag-handle', this._.uid)
      })
    }

    if (this.dragCancel) {
      ;[...this.$el.querySelectorAll(this.dragCancel)].forEach((cancelHandle) => {
        cancelHandle.setAttribute('data-drag-cancel', this._.uid)
      })
    }

    window.addEventListener('resize', this.computeParentRect)
  },

  unmounted() {
    removeEvents(this.domEvents)
    window.removeEventListener('resize', this.computeParentRect)
  },
  ...
  methods: {
    computeParentRect() {
      ...
      this.parentWidth = this.parentW || parentNode.clientWidth
      this.parentHeight = this.parentH || parentNode.clientHeight
    },
    deselect() {
    ...
    bodyDown(ev) {
      ...
      if (this.dragHandle && target?.getAttribute('data-drag-handle') !== this._.uid.toString()) return

      if (this.dragCancel && target?.getAttribute('data-drag-cancel') === this._.uid.toString()) return
      ...
      if (this.isDraggable && !ev.passive) {
        this.bodyDrag = true
      }
    ...
    bodyUp(passive = false) {
      this.bodyDrag = false
      const { x, y, rect } = this
      const { left, top } = rect
      // if (x === left && y === top) return
      this.$emit('dragging', { passive, ...this.rect })
      this.$emit('dragstop', { passive, ...this.rect })
  ...
  watch: {
    ...
    x: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || newVal === this.left) return
        const delta = oldVal - newVal
        this.bodyDown({ passive: true, pageX: this.left, pageY: this.top })
        this.bodyMove({ passive: true, x: delta, y: 0 })
        this.$nextTick(() => {
          this.bodyUp(true)
        }).then()
      }
    ...
    y: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || newVal === this.top) return
        const delta = oldVal - newVal
        this.bodyDown({ passive: true, pageX: this.left, pageY: this.top })
        this.bodyMove({ passive: true, x: 0, y: delta })
    ...
    w: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || newVal === this.width) return
        const stick = 'mr'
        const delta = oldVal - newVal
    ...
    h: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || newVal === this.height) return
        const stick = 'bm'
        const delta = oldVal - newVal
